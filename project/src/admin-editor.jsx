// Admin — Menu editor (desktop)
// Drag&drop sections + dishes, live mobile preview, QR generator.

function AdminEditor({ palette, initialModalDishId = null }) {
  const { sections, dishes: initialDishes, restaurant } = window.MENU_DATA;
  const [dishes, setDishes] = React.useState(initialDishes);
  const [selectedDish, setSelectedDish] = React.useState(initialDishes[3]); // tagliolini
  const [modalDish, setModalDish] = React.useState(
    initialModalDishId ? initialDishes.find((d) => d.id === initialModalDishId) : null
  );
  const [draggingId, setDraggingId] = React.useState(null);
  const [dropTarget, setDropTarget] = React.useState(null);
  const [activeTab, setActiveTab] = React.useState('content'); // content | qr

  // Drag handlers — simple item-into-section drag
  ...
  return (
    <div style={{
      ...
      display: 'grid', gridTemplateColumns: '64px 1fr 380px', overflow: 'hidden',
    }}>
      {/* Mini sidebar */}
      <aside style={{
        ...
        <div style={{
          width: 36, height: 36, background: palette.accent, color: palette.bg,
          borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 18, marginBottom: 12,
        }}>M</div>
        {['M', 'P', 'D', 'Q', 'A', 'S'].map((c, i) => (
          <button key={i} style={{
            width: 36, height: 36, border: 'none', borderRadius: 4, cursor: 'pointer',
      ...
      <main style={{ overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        {/* Breadcrumb / topbar */}
        <header style={{
          padding: '18px 28px', borderBottom: `1px solid ${palette.border}`,
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          background: palette.bg, position: 'sticky', top: 0, zIndex: 5,
        }}>
          <div>
            <div style={{ fontFamily: '"Geist Mono", monospace', fontSize: 10, letterSpacing: '0.18em', color: palette.muted }}>
              MENU / CENA / MODIFICA
            </div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 4 }}>
              <h1 style={{
                fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 26,
                margin: 0, lineHeight: 1, fontWeight: 400,
              }}>Cena</h1>
              <div style={{ fontSize: 11, color: palette.muted }}>· Carta della sera · 12 piatti</div>
            </div>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ display: 'flex', border: `1px solid ${palette.border}`, borderRadius: 4, background: palette.surface }}>
              <button onClick={() => setActiveTab('content')} style={tabBtn(palette, activeTab === 'content')}>Contenuto</button>
              <button onClick={() => setActiveTab('qr')} style={tabBtn(palette, activeTab === 'qr')}>QR</button>
            </div>
        ...
        {/* Body: either content editor or QR */}
        {activeTab === 'content' ? (
              ...
              return (
                <section key={sec.id} style={{ marginBottom: 28 }}>
                  <div style={{
                    ...
                    <span style={{ fontSize: 11, color: palette.muted }}>{sectionDishes.length} piatti</span>
                    <span style={{ flex: 1 }}/>
                    <button style={{
                      ...btnGhost(palette), fontSize: 11, padding: '4px 10px',
                    }}>
                      + Aggiungi piatto
                    </button>
                  </div>
      ...
      {/* Right: dish inspector + live preview */}
      <aside style={{
        borderLeft: `1px solid ${palette.border}`, background: palette.surface,
      ...
      {modalDish && (
        <DishDetailModal palette={palette} dish={modalDish} onClose={() => setModalDish(null)} />
      )}
    </div>
  );
}

function EditorRow({ dish, palette, selected, dragging, dropAbove, onSelect, onDragStart, onDragEnter, onDragEnd }) {
  return (
    <div
      draggable
      ...
      <div style={{ width: 36, height: 36, borderRadius: 3, background: `url("${dish.img}") center/cover`, flexShrink: 0 }}/>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 15, lineHeight: 1.1 }}>
          {dish.name}
        </div>
        <div style={{ fontSize: 10.5, color: palette.muted, marginTop: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {dish.desc}
        </div>
      </div>
      ...
      <div style={{
        ...
        color: palette.ink, fontWeight: 500, width: 56, textAlign: 'right',
      }}>{dish.price.toFixed(2)} €</div>
      <button style={{
        width: 22, height: 22, border: 'none', background: 'transparent',
        color: palette.muted, cursor: 'pointer', borderRadius: 3,
...
function Pill({ text, palette, solid }) {
  return (
    <span style={{
      ...
      border: `1px solid ${solid ? palette.accent : palette.border}`, borderRadius: 2,
    }}>{text}</span>
  );
}
    ...
    <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <Field label="Nome del piatto" palette={palette}>
        <div style={inputStyle(palette)}>
          <span style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 16 }}>{dish.name}</span>
        </div>
      </Field>

      {/* Description */}
      <Field label="Descrizione" palette={palette}>
        <div style={{ ...inputStyle(palette), minHeight: 56, lineHeight: 1.5, fontSize: 12.5 }}>
          {dish.desc}
        </div>
      </Field>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr', gap: 12 }}>
        <Field label="Prezzo" palette={palette}>
          <div style={{ ...inputStyle(palette), fontFamily: '"Geist Mono", monospace', fontVariantNumeric: 'tabular-nums' }}>
      ...
      <Field label="Abbinamento vino" palette={palette}>
        <div style={inputStyle(palette)}>
          <span style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 14 }}>{dish.wine}</span>
        </div>
      </Field>

      {/* Translations status */}
      <div style={{
        padding: 12, background: palette.bg, border: `1px solid ${palette.border}`, borderRadius: 3,
      }}>
        <div style={{ fontSize: 10.5, color: palette.muted, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>
          Traduzioni
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button style={{
            padding: '3px 8px', fontSize: 10, borderRadius: 2,
            border: `1px dashed ${palette.border}`, background: 'transparent',
...
function Field({ label, palette, children }) {
  return (
    <div>
      <div style={{ fontSize: 10.5, color: palette.muted, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>
        {label}
      </div>
      {children}
    </div>
...
function inputStyle(p) {
  return {
    padding: '8px 10px', border: `1px solid ${p.border}`,
    background: p.bg, borderRadius: 3, fontFamily: '"Geist", sans-serif',
...
function tabBtn(p, active) {
  return {
    ...
    background: active ? palette_ink(p) : 'transparent',
    color: active ? p.bg : p.muted,
    border: 'none', borderRadius: 3, fontFamily: 'inherit', fontWeight: 500,
  };
  function palette_ink(p) { return p.ink; }
}

function QRPanel({ palette }) {
  return (
    <div style={{ padding: 32, flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
      <div>
        <div style={{ fontFamily: '"Geist Mono", monospace', fontSize: 10, letterSpacing: '0.22em', color: palette.muted, textTransform: 'uppercase' }}>
          QR per il tavolo
        </div>
        <h2 style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 26, margin: '8px 0 18px', fontWeight: 400 }}>
          Genera codici QR
        </h2>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
              {Array.from({ length: 14 }, (_, i) => (
                <span key={i} style={{
                  ...
                  background: i === 6 ? palette.ink : palette.bg, color: i === 6 ? palette.bg : palette.ink,
                  border: `1px solid ${i === 6 ? palette.ink : palette.border}`, borderRadius: 3,
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['Quadrato', 'Arrotondato', 'Punti'].map((s, i) => (
                <span key={s} style={{
                  ...
                  background: i === 0 ? palette.ink : palette.bg, color: i === 0 ? palette.bg : palette.ink,
                  border: `1px solid ${i === 0 ? palette.ink : palette.border}`, borderRadius: 3,
                }}>{s}</span>
              ))}
            </div>
          </Field>
          <Field label="Colore" palette={palette}>
            <div style={{ display: 'flex', gap: 6 }}>
              {[palette.ink, palette.accent, '#1d3a8a', '#7a3a1a'].map((c, i) => (
                <span key={i} style={{
                  width: 30, height: 30, background: c, borderRadius: 3, cursor: 'pointer',
      ...
      <div style={{
        ...
        borderRadius: 4, padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center',
      }}>
        <div style={{ fontFamily: '"Geist Mono", monospace', fontSize: 10, letterSpacing: '0.22em', color: palette.muted, textTransform: 'uppercase' }}>
          Anteprima · Tavolo 07
        </div>
        ...
        <div style={{ marginTop: 14, fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 20 }}>
          Osteria del Borgo
        </div>
        <div style={{ fontSize: 10.5, color: palette.muted, marginTop: 2, letterSpacing: '0.05em' }}>
          Inquadra per il menu · Scan for menu
        </div>
        <div style={{ marginTop: 22, paddingTop: 18, borderTop: `1px solid ${palette.border}`, width: '100%' }}>
          <div style={{ fontSize: 10.5, color: palette.muted, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>
            Statistiche scansioni
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
            <span>Oggi</span><span style={{ fontFamily: '"Geist Mono", monospace' }}>27</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginTop: 4, color: palette.muted }}>
            <span>Questa settimana</span><span style={{ fontFamily: '"Geist Mono", monospace' }}>184</span>
          </div>
        </div>
      </div>
    </div>
...
function FakeQR({ palette, size = 200 }) {
  // Decorative QR-ish pattern. Deterministic from cell coordinates.
  const cells = 21;
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ background: palette.surface }}>
      {Array.from({ length: cells }).map((_, y) =>
        Array.from({ length: cells }).map((_, x) => {
          ...
          return <rect key={`${x}-${y}`} x={x * cellSize} y={y * cellSize} width={cellSize} height={cellSize} fill={palette.ink}/>;
        })
      )}
      {/* finder squares */}
      ...
      <text x={size / 2} y={size / 2 + 7} textAnchor="middle"
        style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 22, fill: palette.accent }}>
        M
      </text>
    </svg>
  );
...
function LivePreview({ palette }) {
  // Compact preview pane for the QR tab.
  return (
    <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ fontFamily: '"Geist Mono", monospace', fontSize: 10, letterSpacing: '0.22em', color: palette.muted, textTransform: 'uppercase' }}>
        Anteprima cliente
      </div>
      <div style={{ background: palette.bg, border: `1px solid ${palette.border}`, borderRadius: 4, padding: 16 }}>
        <div style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 20, textAlign: 'center' }}>
          Osteria del Borgo
        </div>
        <div style={{ fontSize: 10, color: palette.muted, textAlign: 'center', marginTop: 4 }}>
          Carta della sera
        </div>
        <div style={{ marginTop: 14, padding: '8px 0', borderTop: `1px solid ${palette.border}`, fontSize: 11, lineHeight: 1.5 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 13 }}>Vitello tonnato</span>
            <span style={{ fontFamily: '"Geist Mono", monospace' }}>16,00 €</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            <span style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 13 }}>Burrata e fichi</span>
